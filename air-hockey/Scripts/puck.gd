...
@export var WAV_MALLET_HIT: AudioStream
@export var WAV_WALL_HIT: AudioStream
...
func _physics_process(delta: float) -> void:
	...
	if speed < 200 or Sfx.muted:
		slide_player.volume_db = -80 
	else:
		var target_volume = remap(speed, 200, 2500, -25, 0)
		target_volume = clamp(target_volume, -25, 0)
		slide_player.volume_db = lerp(slide_player.volume_db, target_volume, 15 * delta)
		
		# Pitch shifting the slide: faster sliding = higher friction whine
		var slide_pitch = remap(speed, 200, 2500, 0.9, 1.3)
		slide_player.pitch_scale = clamp(slide_pitch, 0.9, 1.3)
...
func _on_body_entered(body: Node) -> void:
	...
	if speed < 50 or goalscored or Sfx.muted: 
		return 
	
	# Swap the audio clip depending on what we hit
	if body.is_in_group("Paddle"):
		hit_player.stream = WAV_MALLET_HIT
	else:
		hit_player.stream = WAV_WALL_HIT
